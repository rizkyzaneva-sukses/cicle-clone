...
const { ensureBrandProfileFields, ensureProjectReportTables, cleanupOrphanRecords, ensureDefaultWorkspace, backfillProjectMembers, applyAccountHotfixes } = require('./lib/maintenance');
...
app.use(async (req, res, next) => {
  ...
  if (req.session.user) {
    ...
    try {
      maintenancePromise ||= Promise.all([ensureBrandProfileFields(), ensureProjectReportTables(), cleanupOrphanRecords(), backfillProjectMembers(), applyAccountHotfixes()]).catch((err) => {
